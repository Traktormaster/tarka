...
async def test_tx_asqla(tmp_path, db_connect_url):
    ...
    # NOTE: aiosqlite will behave more like a row-locking db with "BEGIN (DEFERRED)" transactions.
    async with start_db(db_connect_url, aiosqlite_serializable_begin="BEGIN") as db:
    ...
    if db_connect_url.startswith("sqlite+aiosqlite://"):
        c = 0
        # check behaviour is serial with BEGIN IMMEDIATE
        engine_kwargs = dict(connect_args={"timeout": 0.5})  # pressure SQLite to yield more tx conflicts
        async with start_db(db_connect_url, engine_kwargs, aiosqlite_serializable_begin="BEGIN IMMEDIATE") as db:
            async with db.engine.connect() as conn:
                async with conn.begin():
                    await conn.execute(delete(Thing))
                    await conn.execute(insert(Thing).values([dict(ext_id=105), dict(ext_id=106), dict(ext_id=108)]))
            ...
            await asyncio.sleep(3)
            assert (await get_value(106)) == 2 + parallel * 2
            assert c > c_prev + parallel  # TODO: should be == 0, but see tx.py:105
            c_prev = c
